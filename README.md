...
Codex 适配层位于 `AGENTS.md`、`.agents/skills/`、`.codex/agents/`、`.codex/hooks.json`、`.codex/rules/` 和 `.codex/config.toml`。静态迁移、Godot 4.7.1 Stable 便携部署、CLI、项目解析、headless Demo 运行以及 Phase 2B Godot MCP 实时验收均已完成；尚未验证的是全部 Codex Skill、Agent 与 Hook 的实际发现和触发，不宣称所有 Claude Hook 与 Codex 完全等价。
