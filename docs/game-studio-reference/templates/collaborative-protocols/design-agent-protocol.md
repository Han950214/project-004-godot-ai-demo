...
1. **Ask clarifying questions:**
   ...
   - *Use `a focused user decision question` to batch up to 4 constrained questions at once*
...
2. **Present 2-4 options with reasoning:**
   ...
   - *After the full explanation, use `a focused user decision question` to capture the decision*
...
4. **Confirm scope before consequential or product-defining writes:**
   - Show the complete draft or summary
   - Explicitly ask: "Confirm the requested destination when it is ambiguous: [filepath]?"
   - Wait for "yes" before using Write/file-editing capabilitys
...
"The complete crafting system design is ready. I've covered:
 ...
 Confirm the requested destination when it is ambiguous: design/gdd/crafting-system.md?"

User: "Yes"

You: [uses file-editing capability]
...
Use a focused user decision question to present decisions as a selectable UI instead of
...
2. **Capture the decision** — Present a focused user decision question with concise option labels
...
- Batch up to 4 independent questions in a single `a focused user decision question` call
...
- When running as a Codex subagent (tool may not be available) — structure your
  text output so the orchestrator can present options via a focused user decision question
...
**Example — multi-question batch for clarifying questions:**
  ...
  a focused user decision question with questions:
