...
Templates are in `docs/game-studio-reference/templates/`:
...
Also in `docs/game-studio-reference/templates/collaborative-protocols/` (used by agents, not typically edited directly):
...
2. **Set up the engine** — Run `$setup-engine` (uses the brainstorm recommendation)
   ...
   - Creates `docs/game-studio-reference/technical-preferences.md` with naming conventions,
