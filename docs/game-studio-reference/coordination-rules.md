...
Skills with ``: `$help`, `$sprint-status`, `$story-readiness`, `$scope-check`,
`$project-stage-detect`, `$changelog`, `$patch-notes`, `$onboard`

Skills with ``: `$review-all-gdds`, `$architecture-review`, `$gate-check`
...
Spawned through Codex subagent delegation within a single Codex session. Used by all `team-*` skills
