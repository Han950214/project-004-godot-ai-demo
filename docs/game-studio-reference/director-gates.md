...
```
Spawn `creative-director` through an optional Codex subagent using gate **CD-PILLARS** from
`docs/game-studio-reference/director-gates.md`.
```
...
Spawn `[agent-name]` through an optional Codex subagent:
- Gate: [GATE-ID] (see docs/game-studio-reference/director-gates.md)
...
Spawn all [N] agents simultaneously through an optional Codex subagent — issue all subagent calls before
...
| **CONCERNS [list]** | Issues present but not blocking. | Surface to user via `a focused user decision question` — options: `Revise flagged items` / `Accept and proceed` / `Discuss further` |
...
- Game pillars and core fantasy
- Platform and performance constraints (from `docs/game-studio-reference/technical-preferences.md` if configured)
