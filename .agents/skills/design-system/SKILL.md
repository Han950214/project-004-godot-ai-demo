---
name: design-system
description: "Guided, section-by-section GDD authoring for a single game system. Gathers context from existing docs, walks through each required section collaboratively, cross-references dependencies, and writes incrementally to file. 当用户明确点名该流程或当前任务与其专业目标直接匹配时使用；不用于无关的一般开发任务。"
---
...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and use `a focused user decision question`:
...
6. If yes: proceed to **Phase 2 (Gather Context)** as normal, but in **Phase 3**
   ...
   incomplete sections.
7. **Never overwrite existing section content.** Use file-editing capability to replace only
...
- **Related GDDs**: file search `design/gdd/*.md` and read any that are thematically related
...
- Read `docs/game-studio-reference/technical-preferences.md` to identify the engine and version
...
- file search `docs/architecture/adr-*.md` and read any ADRs whose domain matches
...
Use `a focused user decision question`:
...
Use the template structure from `docs/game-studio-reference/templates/game-design-document.md`:
...
- Use file search to check if the file exists.
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   `a focused user decision question` for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use `a focused user decision question` to capture the decision.
...
6. **Approval**: Immediately after the draft — in the SAME response — use
   `a focused user decision question`. **NEVER use plain text. NEVER skip this step.**
...
7. **Write**: Use the file-editing capability to replace the placeholder with the approved content.
   **CRITICAL**: Always include the section heading in the `old_string` to ensure
   ...
   same placeholder and the file-editing capability requires a unique match. Use this pattern:
   ```
...
completed section name. Use file search to check if the file exists — use Write to create
...
- **ADR ref tab**: file search `docs/architecture/adr-*.md` and grep for the system name in the GDD Requirements section of any ADR. If a matching ADR is found → `[A] Yes — cite the ADR` recommended. If none found → `[B] No` recommended.
...
**Framing questions (ask BEFORE drafting)**: Use `a focused user decision question` with a multi-tab widget:
...
**Framing question (ask BEFORE drafting)**: Use `a focused user decision question`:
...
spawn `creative-director` through an optional Codex subagent:
...
**Agent delegation (MANDATORY)**: Before drafting Section C, spawn specialist agents through an optional Codex subagent in parallel:
...
- Surface any disagreements between agents to the user via `a focused user decision question`
...
**Agent delegation (MANDATORY)**: Before proposing any formulas or balance values, spawn specialist agents through an optional Codex subagent in parallel:
...
- Present the specialists' proposals to the user for review via `a focused user decision question`
...
For required systems: **spawn `art-director` through an optional Codex subagent** before drafting this section. Provide: system name, game concept, game pillars, art bible sections 1–4 if they exist. Ask them to specify: (1) VFX and visual feedback requirements for this system's events, (2) any animation or visual style constraints, (3) which art bible principles most directly apply to this system. Present their output; do NOT leave this section as `[To be designed]` for visual systems.
...
Before finalizing the GDD, spawn `creative-director` through an optional Codex subagent using gate **CD-GDD-ALIGN** (`docs/game-studio-reference/director-gates.md`).
...
```
`rg` text search pattern="  - name: [candidate_name]" path="design/registry/entities.yaml"
```
...
**When delegating through an optional Codex subagent tool**:
...
- The main session presents the agent's output to the user via `a focused user decision question`
- The user decides; the main session writes to file
...
2. **a focused user decision question** at every decision point (Explain -> Capture pattern):
