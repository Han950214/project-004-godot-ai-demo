---
name: prototype
description: "Concept prototype — validate the core idea is worth designing before writing GDDs. Run right after $brainstorm and $setup-engine. Routes to HTML, Engine, or Paper path based on game type. Produces a throwaway build and a PROCEED/PIVOT/KILL verdict. 当用户明确点名该流程或当前任务与其专业目标直接匹配时使用；不用于无关的一般开发任务。"
---
...
Otherwise, use `a focused user decision question` to confirm intent before proceeding:
...
Read `AGENTS.md` and `docs/game-studio-reference/technical-preferences.md` for the engine and
language in use.
...
Assess which path best fits the hypothesis, then use `a focused user decision question` with your
...
Read `docs/game-studio-reference/templates/prototype-report.md` to get the report structure.
...
- `full` → spawn `creative-director` through an optional Codex subagent using gate **CD-PLAYTEST** if
...
Ask: "Confirm the requested destination when it is ambiguous: `prototypes/[concept-name]-concept/PIVOT-NOTE.md`?"
...
2. **Choose path** — same a focused user decision question widget as Phase 3 (HTML / Engine / Paper).
