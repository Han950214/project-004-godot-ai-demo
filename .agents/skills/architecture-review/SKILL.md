...
Before reading any full document, use `rg` text search to extract `## Summary` sections
from all GDDs and ADRs:

```
`rg` text search pattern="## Summary" glob="design/gdd/*.md" output_mode="content" -A 4
`rg` text search pattern="## Summary" glob="docs/architecture/adr-*.md" output_mode="content" -A 3
```

For `single-gdd [path]` mode: use the target GDD's summary to identify which
ADRs reference the same system (`rg` text search ADRs for the system name), then full-read
...
### Project Standards
- `docs/game-studio-reference/technical-preferences.md`
...
file search `production/epics/**/*.md` (excluding EPIC.md index files). For each
story file:
...
file search `tests/unit/**/*_test.*` and `tests/integration/**/*_test.*`.
Build an index: system → [test file paths].

For each test file path from Step 3b-1, confirm via file search whether the file
...
- `rg` text search all ADRs for API names listed in `deprecated-apis.md`
...
After completing the engine audit above, spawn the **primary engine specialist** through an optional Codex subagent for a domain-expert second opinion:
- Read `docs/game-studio-reference/technical-preferences.md` `Engine Specialists` section to get the primary specialist
...
If [B]: display the complete proposed systems-index section, then re-ask with `a focused user decision question`.
...
Use `a focused user decision question` for the write approval:
...
For `rtm` mode, use `a focused user decision question`:
...
2. **Pre-gate checklist**: Check whether these exist via file search and mark each ✅ or ❌:
...
Then close with `a focused user decision question` tailored to the pre-gate checklist state:
...
3. **Offer options** via a focused user decision question with three choices:
...
5. **Use `a focused user decision question` for write approvals** — plain text "May I?" is not
