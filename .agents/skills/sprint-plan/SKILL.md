---
name: sprint-plan
description: "Generates a new sprint plan or updates an existing one based on the current milestone, completed work, and available capacity. Pulls context from production documents and design backlogs. 当用户明确点名该流程或当前任务与其专业目标直接匹配时使用；不用于无关的一般开发任务。"
---
...
- If the file doesn't exist and this is a `new` sprint: use `a focused user decision question`:
...
3. Ask the user what to change: stories to add, remove, reprioritize, or re-estimate. Use `a focused user decision question` to gather changes.
...
Before finalising the sprint plan, spawn `producer` through an optional Codex subagent using gate **PR-SPRINT** (`docs/game-studio-reference/director-gates.md`).
