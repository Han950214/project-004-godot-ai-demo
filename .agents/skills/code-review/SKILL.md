---
name: code-review
description: "Performs an architectural and quality code review on a specified file or set of files. Checks for coding standard compliance, architectural pattern adherence, SOLID principles, testability, and performance concerns. 当用户明确点名该流程或当前任务与其专业目标直接匹配时使用；不用于无关的一般开发任务。"
---
...
Read `docs/game-studio-reference/technical-preferences.md`, section `## Engine Specialists`. Note:
...
Spawn all applicable specialists simultaneously through an optional Codex subagent — do not wait for one before starting the next.
...
For Logic and Integration stories, also spawn `qa-tester` through an optional Codex subagent in parallel with the engine specialists. Pass:
