---
name: gate-check
description: "Validate readiness to advance between development phases. Produces a PASS/CONCERNS/FAIL verdict with specific blockers and required artifacts. Use when user says 'are we ready to move to X', 'can we advance to production', 'check if we can start the next phase', 'pass the gate'. 当用户明确点名该流程或当前任务与其专业目标直接匹配时使用；不用于无关的一般开发任务。"
---
...
- [ ] Technical preferences configured (`docs/game-studio-reference/technical-preferences.md` populated)
...
### Artifact Checks
- Use `file search` and `Read` to verify files exist and have meaningful content
...
Use `file search('design/gdd/gdd-cross-review-*.md')` to find the `$review-all-gdds` report.
...
- For localization checks: ``rg` text search` for hardcoded strings in `src/`
...
Before generating the final verdict, spawn all four directors as **parallel subagents** through an optional Codex subagent using the parallel gate protocol from `docs/game-studio-reference/director-gates.md`. Issue all four subagent calls simultaneously — do not wait for one before starting the next.

**Spawn in parallel:**

1. **`creative-director`** — gate **CD-PHASE-GATE** (`docs/game-studio-reference/director-gates.md`)
2. **`technical-director`** — gate **TD-PHASE-GATE** (`docs/game-studio-reference/director-gates.md`)
3. **`producer`** — gate **PR-PHASE-GATE** (`docs/game-studio-reference/director-gates.md`)
4. **`art-director`** — gate **AD-PHASE-GATE** (`docs/game-studio-reference/director-gates.md`)
...
> **Tool-action requirement**: At least 2 of the 5 challenge questions below must be answered by re-reading a specific file (Read tool) or re-running a specific check (`rg` text search tool) — not by reflection alone. Mark these with [TOOL ACTION] to indicate a tool was used.
...
After the verdict is presented and any stage.txt update is complete, close with a structured next-step prompt using `a focused user decision question`.
...
- **No Difficulty Curve doc?** → Create `design/difficulty-curve.md` from the template at `docs/game-studio-reference/templates/difficulty-curve.md` — or use `$quick-design "difficulty curve"` for a guided session.
- **No player journey map?** → Create `design/player-journey.md` from the template at `docs/game-studio-reference/templates/player-journey.md` — or author it collaboratively using `$ux-design` Phase 2b.
