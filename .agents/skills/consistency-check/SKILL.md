...
description: "Scan all GDDs against the entity registry to detect cross-document inconsistencies: same entity with different stats, same item with different values, same formula with different variables. `rg` text search-first approach — reads registry then targets only conflicting GDD sections rather than full document reads. 当用户明确点名该流程或当前任务与其专业目标直接匹配时使用；不用于无关的一般开发任务。"
---
...
```
file search pattern="design/gdd/*.md"
```
...
## Phase 3: `rg` text search-First Conflict Scan
...
```
`rg` text search pattern="[entity_name]" glob="design/gdd/*.md" output_mode="content" -C 3
```
...
```
(Or use `rg` text search with wider context if the file is large)
...
Then close with an `a focused user decision question` widget:
