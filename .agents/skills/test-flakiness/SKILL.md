...
For Unreal projects: automation logs go to `Saved/Logs/`. `rg` text search for
...
- `rg` text search for `<testcase name=` to get test names
- `rg` text search for `<failure` or `<error` to identify failures
- Parse `classname` and `name` attributes for full test identifiers

**Plain text logs**:
- `rg` text search for pass/fail patterns:
...
Use `rg` text search to check the test file for timing calls, randf, global state access,
