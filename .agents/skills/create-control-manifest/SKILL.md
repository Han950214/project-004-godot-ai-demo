...
### ADRs
- file search `docs/architecture/adr-*.md` and read every file
...
Spawn `technical-director` through an optional Codex subagent using gate **TD-MANIFEST** (`docs/game-studio-reference/director-gates.md`).
...
- **CONCERNS** → surface via `a focused user decision question` with options: `Revise flagged rules` / `Accept and proceed` / `Discuss further`
