...
3. `docs/game-studio-reference/technical-preferences.md` — naming conventions, performance budgets,
...
Display a one-paragraph summary of what the document will contain (layers, modules, data flows, ADR gaps). Then use `a focused user decision question`:
...
Apply gate **TD-ARCHITECTURE** (`docs/game-studio-reference/director-gates.md`) as a self-review. Check all four criteria from that gate definition against the completed document.
...
**Step 2 — Spawn `lead-programmer` through an optional Codex subagent using gate LP-FEASIBILITY (`docs/game-studio-reference/director-gates.md`):**
...
Use `a focused user decision question` — "Technical Director and Lead Programmer have reviewed the architecture. How would you like to proceed?"
...
Show the proposed Document Status block inline, then use `a focused user decision question`:
