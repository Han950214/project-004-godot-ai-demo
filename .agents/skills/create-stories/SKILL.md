...
in this skill — apply the check pattern from `docs/game-studio-reference/director-gates.md`
...
- No argument — ask: "Which epic would you like to break into stories?"
  file search `production/epics/*/EPIC.md` and list available epics with their status.
...
After decomposing all stories (Step 4 complete) but before presenting them for write approval, spawn `qa-lead` through an optional Codex subagent using gate **QL-STORY-READY** (`docs/game-studio-reference/director-gates.md`).
...
**Before generating test specs**: file search `production/qa/qa-plan-*.md` for the most recently modified file. If found, read it and check whether it contains test case specifications for the stories in this epic (look for story titles or slugs in the plan's Automated Tests Required section). If matching specs exist:
...
Use `a focused user decision question` to close with context-aware next steps:
