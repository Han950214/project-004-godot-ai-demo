---
name: help
description: "Analyzes what is done and the users query and offers advice on what to do next. Use if user says what should I do next or what do I do now or I'm stuck or I don't know what to do 当用户明确点名该流程或当前任务与其专业目标直接匹配时使用；不用于无关的一般开发任务。"
---
...
Read `docs/game-studio-reference/workflow-catalog.yaml`. This is the authoritative list of all
...
After reading the catalog, file search `.agents/skills/*/SKILL.md` to get the full list
...
- Use file search to check if files matching the pattern exist
- If `min_count` is specified, verify at least that many files match
- If `artifact.pattern` is specified, use `rg` text search to verify the pattern exists in the matched file
