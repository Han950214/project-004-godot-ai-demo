...
the same GDD inputs but produce separate reports. Spawn both as parallel subagent
...
- The engine name and version from `docs/game-studio-reference/technical-preferences.md` and `docs/engine-reference/[engine]/VERSION.md`
...
Use `a focused user decision question` for write permission:
- Prompt: "May I write this review to `design/gdd/gdd-cross-review-[date].md`?"
- Options: `[A] Yes — write the report` / `[B] No — skip`

If any GDDs are flagged for revision, use a second `a focused user decision question`:
...
After all file writes are complete, use `a focused user decision question` for a closing widget.
