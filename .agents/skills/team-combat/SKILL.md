---
name: team-combat
description: "Orchestrate the combat team: coordinates game-designer, gameplay-programmer, ai-programmer, technical-artist, sound-designer, and qa-tester to design, implement, and validate a combat feature end-to-end. 仅在用户明确要求团队协作，或任务存在互斥且可独立并行的工作面时使用；不得用于普通单文件修改或自动推进下一制作阶段。"
---
...
- **engine specialist** (primary) — Validate architecture and implementation patterns are idiomatic for the engine (read from `docs/game-studio-reference/technical-preferences.md` Engine Specialists section)
...
delegated to sub-agents spawned through an optional Codex subagent. Each sub-agent enforces the
