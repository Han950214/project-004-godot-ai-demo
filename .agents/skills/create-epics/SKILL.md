---
name: create-epics
description: "Translate approved GDDs + architecture into epics — one epic per architectural module. Defines scope, governing ADRs, engine risk, and untraced requirements. Does NOT break into stories — run $create-stories [epic-slug] after each epic is created. 当用户明确点名该流程或当前任务与其专业目标直接匹配时使用；不用于无关的一般开发任务。"
---
...
`rg` text search all GDDs for their `## Summary` sections before reading anything fully:

```
`rg` text search pattern="## Summary" glob="design/gdd/*.md" output_mode="content" -A 5
```
...
After all epics for the current layer are defined (Step 4 completed for all in-scope systems), and before writing any files, spawn `producer` through an optional Codex subagent using gate **PR-EPIC** (`docs/game-studio-reference/director-gates.md`).
...
If CONCERNS, use `a focused user decision question`:
