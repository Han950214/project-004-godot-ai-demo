---
name: tech-debt
description: "Track, categorize, and prioritize technical debt across the codebase. Scans for debt indicators, maintains a debt register, and recommends repayment scheduling. 当用户明确点名该流程或当前任务与其专业目标直接匹配时使用；不用于无关的一般开发任务。"
---
...
Then use `a focused user decision question` to collect the **category**:
...
Then use `a focused user decision question` to collect the **estimated fix effort**:
