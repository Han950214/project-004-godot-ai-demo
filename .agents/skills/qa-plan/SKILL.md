---
name: qa-plan
description: "Generate a QA test plan for a sprint or feature. Reads GDDs and story files, classifies stories by test type (Logic/Integration/Visual/UI), and produces a structured test plan covering automated tests required, manual test cases, smoke test scope, and playtest sign-off requirements. Run before sprint begins or when starting a major feature. 当用户明确点名该流程或当前任务与其专业目标直接匹配时使用；不用于无关的一般开发任务。"
---
...
**Argument:** `$ARGUMENTS` (blank = ask user via a focused user decision question)
...
- **No argument** — use `a focused user decision question`:
...
**Engine**: [engine name from docs/game-studio-reference/technical-preferences.md, or "Not configured"]
...
then ask two questions together using `a focused user decision question`:
...
- Use `a focused user decision question` for scope selection when no argument is provided.
