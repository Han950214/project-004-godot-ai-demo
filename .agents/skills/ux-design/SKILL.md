---
name: ux-design
description: "Guided, section-by-section UX spec authoring for a screen, flow, or HUD. Reads game concept, player journey, and relevant GDDs to provide context-aware design guidance. Produces ux-spec.md (per screen/flow) or hud-design.md using the studio templates. 当用户明确点名该流程或当前任务与其专业目标直接匹配时使用；不用于无关的一般开发任务。"
---
...
**If no argument is provided**, do not fail — ask instead. Use `a focused user decision question`:
...
> "Player journey map not yet created. Template available at `docs/game-studio-reference/templates/player-journey.md`. Run `$ux-design` Phase 2b or create it manually to establish player context for this screen."

### 2c: GDD UI Requirements

file search `design/gdd/*.md` and grep for `UI Requirements` sections. Read any GDD whose
...
file search `design/ux/*.md` and note which screens already have specs. For screens that
...
Read `docs/game-studio-reference/technical-preferences.md` and extract the `## Input & Platform`
...
file search `design/ux/[filename].md` (where `[filename]` is the resolved output path from Phase 1).
...
2. **Questions**: Ask what is needed to draft this section. Use `a focused user decision question`
...
3. **Options**: Where design choices exist, present 2-4 approaches with pros/cons.
   Explain reasoning in conversation, then use `a focused user decision question` to capture the decision.
4. **Decision**: User picks an approach or provides custom direction.
5. **Draft**: Write the section content in conversation for review. Flag provisional
   assumptions explicitly.
6. **Approval**: Use `a focused user decision question`:
...
7. **Write**: Use `a focused user decision question`: "May I write the [section name] section to `[filepath]`?"
...
- Use `a focused user decision question` to capture the choice:
...
- Use `a focused user decision question`: "Want an ASCII wireframe as part of this spec?"
...
Use `a focused user decision question` to confirm:
...
Use `a focused user decision question` to step through items in groups of 3-4, not all at once.
...
file search `design/ux/*.md` (excluding `interaction-patterns.md`) and read the Component
...
Work through patterns in groups. Use `a focused user decision question`:
...
Then use `a focused user decision question`:
