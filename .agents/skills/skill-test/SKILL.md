---
name: skill-test
description: "Validate skill files for structural compliance and behavioral correctness. Three modes: static (linter), spec (behavioral), audit (coverage report). 当用户明确点名该流程或当前任务与其专业目标直接匹配时使用；不用于无关的一般开发任务。"
---
...
- `name:`
- `description:`
- `default prompt guidance:`
- `explicit invocation:`
- `Codex tool requirements:`
...
**FAIL** if `Codex tool requirements` includes `Write` or `Edit` but no ask-before-write language is found.
...
`default prompt guidance` must be non-empty. If the skill body mentions multiple modes
