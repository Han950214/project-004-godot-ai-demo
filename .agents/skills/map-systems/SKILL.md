---
name: map-systems
description: "Decompose a game concept into individual systems, map dependencies, prioritize design order, and create the systems index. 当用户明确点名该流程或当前任务与其专业目标直接匹配时使用；不用于无关的一般开发任务。"
---
...
- file search `design/gdd/*.md` — check which system GDDs already exist

**If the systems index already exists:**
- Read it and present current status to the user
- Use `a focused user decision question` to ask:
...
Then use `a focused user decision question` to capture feedback:
...
Use `a focused user decision question` to ask: "Does this dependency ordering look right? Any
...
**After dependency mapping is approved, spawn `technical-director` through an optional Codex subagent using gate TD-SYSTEM-BOUNDARY (`docs/game-studio-reference/director-gates.md`) before proceeding to priority assignment.**
...
Use `a focused user decision question` to ask: "Do these priority assignments match your vision?
...
**After priorities are approved, spawn `producer` through an optional Codex subagent using gate PR-SCOPE (`docs/game-studio-reference/director-gates.md`) before writing the index.**
...
Using the template at `docs/game-studio-reference/templates/systems-index.md`, populate the
...
**After the systems index is written, spawn `creative-director` through an optional Codex subagent using gate CD-SYSTEMS (`docs/game-studio-reference/director-gates.md`).**
...
Use `a focused user decision question` for: "Start designing [system-name] now, pick a different
system, or stop here?"
...
After `$design-system` completes, use `a focused user decision question`:
...
After the systems index is created (or after designing some systems), present next actions using `a focused user decision question`:
