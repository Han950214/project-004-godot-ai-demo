...
description: "Map test coverage to GDD critical paths, identify fixed bugs without regression tests, flag coverage drift from new features, and maintain tests/regression-suite.md. Run after implementing a bug fix or before a release gate. 当用户明确点名该流程或当前任务与其专业目标直接匹配时使用；不用于无关的一般开发任务。"
---
...
- `$regression-suite report` — read-only status report (no writes); suitable
  for sprint reviews
- No argument — if a sprint is clearly active (sprint plan exists with in-progress stories), run `update`. If ambiguous or no active sprint is detected, use `a focused user decision question`:
...
file search `production/qa/bugs/*.md` and filter for bugs with a `Status: Closed`
...
1. `rg` text search `tests/unit/[system]/` and `tests/integration/[system]/` for file names
...
2. `rg` text search `tests/unit/[system]/` and `tests/integration/[system]/` for a test
...
- GDD sections added or revised since the regression suite was last updated
  (use `rg` text search on GDD file modification hints if available, or ask the user)
