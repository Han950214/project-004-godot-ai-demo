...
description: "Creates an Architecture Decision Record (ADR) documenting a significant technical decision, its context, alternatives considered, and consequences. Every major technical choice should have an ADR. 当用户明确点名该流程或当前任务与其专业目标直接匹配时使用；不用于无关的一般开发任务。"
---
...
5. If yes:
   ...
   - Append each missing section to the ADR file using the file-editing capability.
...
a **confirm/adjust** prompt using `a focused user decision question` — not open-ended questions.
...
decisions remain, present each one as a separate `a focused user decision question` with the proposed
...
5.5. **Engine Specialist Validation** — Before saving, spawn the **primary engine specialist** through an optional Codex subagent to validate the drafted ADR:
...
5.6. **Technical Director Strategic Review** — After the engine specialist validation, spawn `technical-director` through an optional Codex subagent using gate **TD-ADR** (`docs/game-studio-reference/director-gates.md`):
...
5. **Write approval** — Use `a focused user decision question`:
...
Ask using `a focused user decision question`:
...
After the ADR is written (and registry optionally updated), close with `a focused user decision question`.
