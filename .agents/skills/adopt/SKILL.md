...
- `docs/game-studio-reference/technical-preferences.md` — engine configured?
- `docs/engine-reference/` — engine reference docs present?
- file search `docs/adoption-plan-*.md` — note the filename of the most recent prior plan if any exist
...
If the project appears fresh (no artifacts at all), use `a focused user decision question`:
...
1. **Parenthetical status values** — `rg` text search for any Status cell containing
...
Read `docs/game-studio-reference/technical-preferences.md`. Check each field for `[TO BE CONFIGURED]`:
...
and offer to handle it immediately using `a focused user decision question`. Choose the first
branch that applies:
