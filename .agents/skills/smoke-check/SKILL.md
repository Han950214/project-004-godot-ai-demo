---
name: smoke-check
description: "Run the critical path smoke test gate before QA hand-off. Executes the automated test suite, verifies core functionality, and produces a PASS/FAIL report. Run after a sprint's stories are implemented and before manual QA begins. A failed smoke check means the build is not ready for QA. 当用户明确点名该流程或当前任务与其专业目标直接匹配时使用；不用于无关的一般开发任务。"
---
...
3. **Engine detection**: read `docs/game-studio-reference/technical-preferences.md` and
...
Attempt to run the test suite via shell. Select the command based on the engine
detected in Phase 1:
...
"Engine not configured in `docs/game-studio-reference/technical-preferences.md`. Run
...
Use `a focused user decision question` to batch-verify. Keep to at most 3 calls.
...
- Use `a focused user decision question` for all manual smoke check verification.
