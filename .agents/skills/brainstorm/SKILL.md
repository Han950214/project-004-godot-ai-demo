---
name: brainstorm
description: "Guided game concept ideation — from zero idea to a structured game concept document. Uses professional studio ideation techniques, player psychology frameworks, and structured creative exploration. 当用户明确点名该流程或当前任务与其专业目标直接匹配时使用；不用于无关的一般开发任务。"
---
...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Use `a focused user decision question`** at key decision points throughout brainstorming:
   ...
   `a focused user decision question` to capture the decision with concise labels.
...
- What 3 games have you spent the most time with? What kept you coming back?
  *(Ask this as plain text — the user must be able to type specific game names freely.
  Do NOT put this in an a focused user decision question with preset options.)*
- Are there genres you love? Genres you avoid? Why?
- Do you prefer games that challenge you, relax you, tell you stories,
  or let you express yourself? *(Use `a focused user decision question` for this — constrained choice.)*

**Practical constraints** (shape the sandbox before brainstorming).
Bundle these into a single multi-tab `a focused user decision question` with these exact tab labels:
...
Present all three. Then use `a focused user decision question` to capture the selection.

**CRITICAL**: This MUST be a plain list call — no tabs, no form fields. Use exactly this structure:

```
a focused user decision question(
...
Ask these as `a focused user decision question` calls — derive the options from the chosen concept, don't hardcode them:
...
**Pillar confirmation**: After presenting the full pillar set, use `a focused user decision question`:
...
If the user selects B, C, or D, make the revision, then use `a focused user decision question` again:
...
**After pillars and anti-pillars are agreed, spawn BOTH `creative-director` AND `art-director` through an optional Codex subagent in parallel before moving to Phase 5. Issue both subagent calls simultaneously — do not wait for one before starting the other.**

- **`creative-director`** — gate **CD-PILLARS** (`docs/game-studio-reference/director-gates.md`)
  Pass: full pillar set with design tests, anti-pillars, core fantasy, unique hook.

- **`art-director`** — gate **AD-CONCEPT-VISUAL** (`docs/game-studio-reference/director-gates.md`)
...
Collect both verdicts, then present them together using a two-tab `a focused user decision question`:
...
- **Target platform**: Use `a focused user decision question` — "What platforms are you targeting for this game?"
...
- **Engine experience**: Use `a focused user decision question` — "Do you already have an engine you work in?"
...
**After identifying biggest technical risks, spawn `technical-director` through an optional Codex subagent using gate TD-FEASIBILITY (`docs/game-studio-reference/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` through an optional Codex subagent using gate PR-SCOPE (`docs/game-studio-reference/director-gates.md`).**
...
4. **Generate the game concept document** using the template at
   `docs/game-studio-reference/templates/game-concept.md`. Fill in ALL sections from the
...
5. Use `a focused user decision question` for write approval:
...
If [B]: ask which section to revise using `a focused user decision question` with options: `Elevator Pitch` / `Core Fantasy & Unique Hook` / `Pillars` / `Core Loop` / `MVP Definition` / `Scope Tiers` / `Risks` / `Something else — I'll describe`

After revising, show the updated section as a diff or clear before/after, then use `a focused user decision question` — "Ready to write the updated concept document?"
Options: `[A] Yes — write it` / `[B] Revise another section`
Repeat until the user selects [A].

If yes, generate the document using the template at `docs/game-studio-reference/templates/game-concept.md`, fill in ALL sections from the brainstorm conversation, and write the file, creating directories as needed.
