...
Spawn `qa-lead` through an optional Codex subagent to review all in-scope stories and produce a QA strategy.
...
Present the qa-lead's full strategy to the user, then use `a focused user decision question`:
...
Spawn `qa-tester` through an optional Codex subagent for each story (run in parallel where possible), providing:
- The story file path
...
Use `a focused user decision question` per story group (batched 3-4 at a time):
...
Batch stories into groups of 3-4 and use `a focused user decision question` for each:
...
After each FAIL result: use `a focused user decision question` to collect the failure description, then spawn `qa-tester` through an optional Codex subagent to write a formal bug report in `production/qa/bugs/`.
...
Spawn `qa-lead` through an optional Codex subagent to produce the sign-off report using all results from Phases 4–6.
