---
name: design-review
description: "Reviews a game design document for completeness, internal consistency, implementability, and adherence to project design standards. Run this before handing a design document to programmers. 当用户明确点名该流程或当前任务与其专业目标直接匹配时使用；不用于无关的一般开发任务。"
---
...
**Dependency graph validation:** For every system listed in the Dependencies section, use file search to check whether its GDD file exists in `design/gdd/`. Flag any that don't exist yet — these are broken references that downstream authors will hit.
...
| Engine-specific patterns or APIs | Primary engine specialist (from `docs/game-studio-reference/technical-preferences.md`) |
...
Use `a focused user decision question` for ALL closing interactions. Never plain text.
...
Work through all blocking items, asking for design decisions only where you cannot resolve the issue from the GDD and existing docs alone. Group all design-decision questions into a single multi-tab `a focused user decision question` before making any edits — do not interrupt mid-revision for each blocker individually.

After all revisions are complete, show a summary table (blocker → fix applied) and use `a focused user decision question` for a **post-revision closing widget**:
...
When the verdict is APPROVED, use a single `a focused user decision question` with `multiSelect: true` to batch the two tracking updates:
...
Use a second `a focused user decision question`:
...
Use a third `a focused user decision question`:
...
Once the systems-index and review-log widgets are answered, check project state and show one final `a focused user decision question`:
