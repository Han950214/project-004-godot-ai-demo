---
name: retrospective
description: "Generates a sprint or milestone retrospective by analyzing completed work, velocity, blockers, and patterns. Produces actionable insights for the next iteration. 当用户明确点名该流程或当前任务与其专业目标直接匹配时使用；不用于无关的一般开发任务。"
---
...
If a matching file is found, use `a focused user decision question`:
...
Then use `a focused user decision question` to present two options:
...
Run git log for the sprint period to understand what was actually committed and when. Use the shell tool (which uses Git shell on Windows — the `2>/dev/null` is bash syntax, not PowerShell):

```
shell: git log --oneline --since="4 weeks ago" 2>/dev/null || git log --oneline -20
```
...
Ask: "Confirm the requested destination when it is ambiguous: `production/retrospectives/retro-sprint-[N]-[date].md`?" (or `production/retrospectives/retro-[milestone-name]-[date].md` for milestone retrospectives)
