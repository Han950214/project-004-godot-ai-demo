...
- If it exists: read it, find the first entity or screen with status "Needed" but no spec file yet, and use `a focused user decision question`:
...
Present the full proposed inventory to the user in conversation. Then use `a focused user decision question`:
...
- **Technical preferences**: Read `docs/game-studio-reference/technical-preferences.md` — extract performance budgets and naming conventions.
...
- **system**: Read `design/gdd/[target-name].md`. Extract the **Visual/Audio Requirements** section. If it doesn't exist or reads `[To be designed]`:
  ...
  Use `a focused user decision question`: `[A] Describe needs manually` / `[B] Stop — complete the GDD first`
...
- **character** or **entity**: Read `design/narrative/characters/[target-name].md` or search `design/narrative/` and `design/assets/entity-inventory.md` for a matching entry. Extract visual description, role, and any specified distinguishing features.
  - **If no source doc exists**: do not fail. Instead, use `a focused user decision question`:
...
- **Related specs**: file search `design/assets/specs/*.md` — scan for assets that could be shared (e.g., a common UI element specced for one system might apply here too).
...
Present the full identified list to the user. Use `a focused user decision question`:
...
**`art-director`** through an optional Codex subagent:
...
**`technical-artist`** through an optional Codex subagent:
...
After presenting all specs, use `a focused user decision question`:
...
```
`rg` text search pattern="ASSET-" path="design/assets/asset-manifest.md"
```
