...
description: "Generates a comprehensive milestone progress review including feature completeness, quality metrics, risk assessment, and go/no-go recommendation. Use at milestone checkpoints or when evaluating readiness for a milestone deadline. 当用户明确点名该流程或当前任务与其专业目标直接匹配时使用；不用于无关的一般开发任务。"
---
...
Before generating the Go/No-Go recommendation, spawn `producer` through an optional Codex subagent using gate **PR-MILESTONE** (`docs/game-studio-reference/director-gates.md`).
...
If OFF TRACK, use `a focused user decision question` before generating the recommendation:
...
If AT RISK, use `a focused user decision question`:
...
Ask: "Confirm the requested destination when it is ambiguous: `production/milestones/[milestone-name]-review.md`?"
