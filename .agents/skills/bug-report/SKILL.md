---
name: bug-report
description: "Creates a structured bug report from a description, or analyzes code to identify potential bugs. Ensures every bug report has full reproduction steps, severity assessment, and context. 当用户明确点名该流程或当前任务与其专业目标直接匹配时使用；不用于无关的一般开发任务。"
---
...
2. **Search the codebase** for related files using `rg` text search/file search to add context (affected system, likely files).
...
1. **Re-run reproduction steps** — use `rg` text search/file search to check whether the root cause code path still exists as described. If the fix removed or changed it, note the change.
2. **Run the related test** — if the bug's system has a test file in `tests/`, run it via shell and report pass/fail.
...
Ask: "Confirm the requested destination when it is ambiguous: `production/qa/bugs/BUG-[NNNN].md`?"
