...
See `docs/game-studio-reference/director-gates.md` for the full check pattern and mode definitions.

---

## 1. Parse Arguments

**Scope:** `$ARGUMENTS[0]` (blank = ask user via a focused user decision question)
...
If no argument is given, use `a focused user decision question`:
...
- [ ] **Referenced assets exist**: Scan the story text for asset path patterns
  ...
  - For each asset path found: use file search to check whether the file exists.
...
in conversation. Do not use Write or file-editing capabilitys — the user (or
...
Spawn `qa-lead` through an optional Codex subagent using gate **QL-STORY-READY** (`docs/game-studio-reference/director-gates.md`).
...
- **GAPS [list]** → surface the specific gaps to the user via `a focused user decision question`:
