---
name: setup-engine
description: "Configure the project's game engine and version. Pins the engine in AGENTS.md, detects knowledge gaps, and populates engine reference docs via web search when the version is beyond the LLM's training data. 当用户明确点名该流程或当前任务与其专业目标直接匹配时使用；不用于无关的一般开发任务。"
---
...
**Question 1 — Prior experience** (ask this first, always, via `a focused user decision question`):
...
**Question 2 — Target platform** (ask this second, always, via `a focused user decision question` — platform eliminates or heavily weights engines before any other factor):
...
5. Use `a focused user decision question` to confirm: "Does this recommendation feel right, or would you like to explore a different engine?"
...
Use `a focused user decision question` with concept-specific deep-dive topics. Always generate these options from the user's actual concept — do not use generic options. Always include at minimum:
...
After updating AGENTS.md, create or update `docs/game-studio-reference/technical-preferences.md` with
...
### Remaining Sections
- **Performance Budgets**: Use `a focused user decision question`:
...
Use web search and open the referenced web page to locate the official migration guide between
...
- Use `rg` text search to search for deprecated API names extracted from the migration
