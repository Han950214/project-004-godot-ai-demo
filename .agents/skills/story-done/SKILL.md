---
name: story-done
description: "End-of-story completion review. Reads the story file, verifies each acceptance criterion against the implementation, checks for GDD/ADR deviations, prompts code review, updates story status to Complete, and surfaces the next ready story from the sprint. 当用户明确点名该流程或当前任务与其专业目标直接匹配时使用；不用于无关的一般开发任务。"
---
...
3. If multiple in-progress stories are found, use `a focused user decision question`:
...
- **Test pass check**: if a test file path is mentioned, run it via `shell`.
- **No hardcoded values check**: ``rg` text search` for numeric literals in gameplay code
  paths that should be in config files.
- **No hardcoded strings check**: ``rg` text search` for player-facing strings in `src/`
  that should be in localization files.
- **Dependency check**: if a criterion says "depends on X", check that X exists.

### Manual verification with confirmation (use `a focused user decision question`)
...
Batch up to 4 manual verification questions into a single `a focused user decision question` call:
...
1. Ask: is there a test — unit, integration, or confirmed manual playtest — that
   ...
   - **Unit test**: check `tests/unit/` for a test file or function name that
     matches the criterion's subject (use `file search` and ``rg` text search`)
   - **Integration test**: check `tests/integration/` similarly
   - **Manual confirmation**: if the criterion was verified via `a focused user decision question`
...
- If found: read the file and check the sign-off table for unchecked boxes. `rg` text search for lines matching `| .* | .* | .* | \[ \] Approved` (a sign-off row with an unchecked checkbox). If any unchecked sign-off rows are found: flag as **ADVISORY** — "Evidence file found at `[path]` but [N] sign-off(s) are still pending (shown as `[ ] Approved` in the sign-off table). Obtain required sign-offs before final closure. Note: for solo developers, all roles may be signed off by the same person."
...
1. **GDD rules check**: Using the current requirement text from `tr-registry.yaml`
   ...
   ``rg` text search` the implemented files for key function names, data structures, or class
...
3. **ADR constraints check**: Read the referenced ADR's Decision section. Check
   ...
   exists). ``rg` text search` for patterns explicitly forbidden in the ADR.

4. **Hardcoded values check**: ``rg` text search` the implemented files for numeric literals
...
After completing the deviation checks in Phase 4, spawn `qa-lead` through an optional Codex subagent using gate **QL-TEST-COVERAGE** (`docs/game-studio-reference/director-gates.md`).
...
- `lean` → use `a focused user decision question` before proceeding:
...
Spawn `lead-programmer` through an optional Codex subagent using gate **LP-CODE-REVIEW** (`docs/game-studio-reference/director-gates.md`).

Pass: implementation file paths, story file path, relevant GDD section, governing ADR.

Present the verdict to the user. If CONCERNS, surface them via `a focused user decision question`:
...
Use `a focused user decision question` before writing anything:
...
- Use `a focused user decision question` for the code review prompt and for batching manual
